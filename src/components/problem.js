import Link from "next/link"

export const Problem = () => {

    return (
        <>
        <Link href='/problem'className="fixed h-auto bottom-0 right-3 transform rotate-90 origin-right m-auto">
            <p className='font-montrealbold text-light-accent-color underline
            iphonese:text-xs iphone6:text-xs iphoneplus:text-xs iphonexrmax:text-sm ipad:text-md ipadpro:text-md ipadpro12:text-lg
            dark:text-dark-accent-color'>DESIGN IS A PROBLEM.</p>
        </Link>
        </>
    )
}